...
func TestClient_GetAuthURL(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := &Client{}
			got, err := c.GetAuthURL(tt.args.requestToken, tt.args.redirectUrl)
			if tt.wantErr {
				assert.Error(t, err)
			} else {
...
func TestClient_GetRequestToken(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedResponse:   "code=1713alazag",
			want:               "1713alazag",
		},
		{
...
func TestClient_Auth(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedResponse:   "access_token=1317alazag&username=testuser",
			expectedStatusCode: 200,
			want: &AuthResponse{
				AccessToken: "1317alazag",
				Username:    "testuser",
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := c.Auth(tt.args.ctx, tt.args.requestToken)
			if tt.wantErr {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
		...
		},
		{
			...
			args: args{
				...
				input: AddInput{
					...
					AccessToken: "token",
					Title:       "pepega",
					Tags:        []string{"sera", "was", "never", "an", "agreeble", "girl"},
				},
			},
			expectedStatusCode: 200,
		},
